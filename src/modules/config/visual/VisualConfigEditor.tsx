...
import { VisualConfigCollapsibleSection } from "@/modules/config/visual/VisualConfigCollapsibleSection";
import { VisualConfigSearchBar } from "@/modules/config/visual/VisualConfigSearchBar";
import { VisualConfigSectionNav } from "@/modules/config/visual/VisualConfigSectionNav";
import {
...
function scrollSectionIntoView(element: HTMLElement) {
  try {
    ...
    while (parent) {
      ...
      if (overflowY === "auto" || overflowY === "scroll") {
        ...
        parent.scrollTo({
          top: parent.scrollTop + (elementRect.top - parentRect.top) - 12,
          behavior,
        });
        return;
      }
    ...
    // JSDOM and other test environments may not implement scroll APIs.
  }
}
...
export function VisualConfigEditor({
  ...
  const [expandedSections, setExpandedSections] = useState<Set<VisualConfigSectionId>>(
    readDefaultExpandedSections,
  );
  const [activeSection, setActiveSection] = useState<VisualConfigSectionId | null>("basics");
  const [searchQuery, setSearchQuery] = useState("");
  const [lastSearchedQuery, setLastSearchedQuery] = useState("");
  const [searchIndex, setSearchIndex] = useState(0);
  const sectionRefs = useRef<Partial<Record<VisualConfigSectionId, HTMLElement | null>>>({});

  const matchingSections = useMemo(
    () => findMatchingSections(searchQuery, t),
    [searchQuery, t],
  );

  const searchStats = useMemo(() => {
    const q = searchQuery.trim();
    if (!q || lastSearchedQuery !== q || !matchingSections.length) {
      return { current: 0, total: 0 };
    }
    return { current: searchIndex + 1, total: matchingSections.length };
  }, [lastSearchedQuery, matchingSections.length, searchIndex, searchQuery]);

  const highlightedSection = useMemo(() => {
    const q = searchQuery.trim();
    if (!q || lastSearchedQuery !== q || !matchingSections.length) return null;
    return matchingSections[searchIndex] ?? null;
  }, [lastSearchedQuery, matchingSections, searchIndex, searchQuery]);

  const isExpanded = useCallback(
    (id: VisualConfigSectionId) => expandedSections.has(id),
    [expandedSections],
  );

  const setSectionExpanded = useCallback((id: VisualConfigSectionId, open: boolean) => {
  ...
  const scrollToSection = useCallback((id: VisualConfigSectionId) => {
    setExpandedSections(new Set([id]));
    setActiveSection(id);
    window.setTimeout(() => {
      const node = sectionRefs.current[id];
      if (node) scrollSectionIntoView(node);
    }, 0);
  }, []);
  ...
  const executeSearch = useCallback(
    (direction: "next" | "prev" = "next") => {
      ...
      const matches = findMatchingSections(q, t);
      if (!matches.length) {
      ...
      scrollToSection(matches[nextIndex]);
    },
    [lastSearchedQuery, scrollToSection, searchIndex, searchQuery, t],
  );
  ...
  const sectionProps = (id: VisualConfigSectionId, titleKey: string, descKey?: string) => ({
    id,
    open: isExpanded(id),
    onOpenChange: (open: boolean) => setSectionExpanded(id, open),
    toggleLabel: isExpanded(id) ? t("visual_config.collapse") : t("visual_config.expand"),
    highlighted: highlightedSection === id,
    sectionRef: (node: HTMLElement | null) => {
    ...
    <div className="flex min-h-0 flex-col gap-4 lg:flex-row lg:items-start">
      ...
      <div className="min-w-0 flex-1 space-y-4">
        ...
        <div className="space-y-4">
          <VisualConfigCollapsibleSection
            {...sectionProps("basics", "visual_config.basics", "visual_config.basics_desc")}
          >
            <div className="space-y-4">
          ...
          <VisualConfigCollapsibleSection
            {...sectionProps("tls", "visual_config.tls", "visual_config.tls_desc")}
          >
            <div className="space-y-4">
              <ToggleSwitch
          ...
          <VisualConfigCollapsibleSection
            {...sectionProps(
              "remote",
              "visual_config.remote_mgmt",
              "visual_config.remote_desc",
            )}
          >
          ...
          <VisualConfigCollapsibleSection
            {...sectionProps("cors", "visual_config.cors_title", "visual_config.cors_desc")}
          >
          ...
          <VisualConfigCollapsibleSection
            {...sectionProps("switches", "visual_config.switches", "visual_config.runtime_desc")}
          >
            <div className="space-y-4">
              <ToggleSwitch
          ...
          <VisualConfigCollapsibleSection
            {...sectionProps(
              "kimi-headers",
              "visual_config.kimi_headers",
              "visual_config.kimi_headers_desc",
            )}
          >
          ...
          <VisualConfigCollapsibleSection
            {...sectionProps(
              "payload-default",
              "visual_config.payload_default",
              "visual_config.payload_default_desc",
            )}
          >
            <PayloadRulesEditor
          ...
          <VisualConfigCollapsibleSection
            {...sectionProps(
              "payload-override",
              "visual_config.payload_override",
              "visual_config.payload_override_desc",
            )}
          >
            <PayloadRulesEditor
          ...
          <VisualConfigCollapsibleSection
            {...sectionProps(
              "payload-filter",
              "visual_config.payload_filter",
              "visual_config.payload_filter_desc",
            )}
          >
            <PayloadFilterRulesEditor
